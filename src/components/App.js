...
const [coupon,setcoupon]=useState(false);
  return (
    <div >
      <div className="navbar-expand-lg" id="all-products">
      ...
      {cart.length>0 && cart.map((item,i)=>{
        ...
        <div>
          ...
          <div className="last-btn card-body"><button className="btn" onClick={()=>handleincrement('dec',i,item)}>-</button><span>{count[i]}</span><button className="btn" onClick={()=>handleincrement('inc',i,item)}>+</button></div>
          </div>
        <div className="btn">
