  ...
  return (
    <div >
      ...
      {wishlist.length>0 && wishlist.map((item,i)=>{
       return <div className="item" key={'w'+i}>
          <img src={item.images[0]} />
          <p>${item.price}</p>
          <p>{item.title}</p>
          <button onClick={()=>handleaddtocart(item,'wishlist')}>Add to Cart</button>
       </div>
      })}
      </div>
      </div>
      ...
      {cart.length>0 && cart.map((item,i)=>{
        ...
        return<div className="cart" key={'cart'+i}>
        <img src={item.images[0]} />
        <div>
          <div className="p-lastbtn">
          <div className="details"><p>Price:<br/>${item.price}</p>
          <p>Product Name:<br/>{item.title}</p></div>
          <div className="last-btn"><button onClick={()=>handleincrement('dec',i,item)}>-</button><span>{count[i]}</span><button onClick={()=>handleincrement('inc',i,item)}>+</button></div>
          </div>
        <div className="btn">
...
<button onClick={()=>handlecartremove(item)}>Remove item</button></div>
<div><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
  <path strokeLinecap="round" strokeLinejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12Z" />
</svg><button onClick={()=>handlewishlist(item)}>Move to Wishlist</button></div>
</div>
        </div>
      </div>}
        )}
