      ...
      <div className="all">
        <div className="col-lg-8">
          ...
          {all.length > 0 ? all.map((item, i) => (
            <div className="item custom-card card" key={'a' + i}>
        ...
        <div>
          ...
          <div id="wishlists" className="all col-lg-8">
            {wishlist.length > 0 ? wishlist.map((item, i) => (
              <div className="item custom-card card" key={'w' + i}>
                <img src={item.images[0]} alt={item.title} />
                <p>${item.price}</p>
                <h4>{item.title}</h4>
      ...
      <div className="cart-total ">
        <div id="cart" className="col-lg-8">
          ...
          {cart.length > 0 ? cart.map((item, i) => (
            <div className="cart custom-card card" key={'cart' + i}>
              <img src={item.images[0]} alt={item.title} />
              <div className="card-body">
                <div className="details">
                  <p>Price:<br />${item.price}</p>
                  <h5>Product Name:<br />{item.title}</h5>
                </div>
                <button className="btn" onClick={() => handleincrement('dec', i, item)}>-</button>
                {count[i]}
                <button className="btn" onClick={() => handleincrement('inc', i, item)}>+</button>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
                  <path strokeLinecap="round" strokeLinejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                </svg>
                <button className="btn btn-primary" onClick={() => handlecartremove(item)}>Remove item</button>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12Z" />
                </svg>
                <button className="btn" onClick={() => handlewishlist(item)}>Move to Wishlist</button>
              </div>
            </div>
        ...
        <div className="side-total col-lg-4">
          <div className="custom-card card">
            ...
            <hr />
            {cart.length > 0 ? cart.map((item, i) => (
              <div className="itemprice" key={'total' + i}>{i + 1}. {item.title} <span className="px-4">${parseInt(item.price) * parseInt(count[i])}</span></div>
            )) : <p>No items in cart</p>}
            <div className="total-price">Total Amount: <span className="price px-4">${price}</span></div>
          </div>
          <button className="btn btn-primary" onClick={() => setcoupon(true)}>Apply coupon</button>
        </div>
      </div>
